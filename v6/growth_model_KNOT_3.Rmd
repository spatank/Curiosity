...
all_data$GenderFactor <- as.factor(all_data$GenderFactor)
```
...
```{r}
table(all_data$GenderFactor)
```


```{r}
betti_cur <- lme(fixed = Betti ~ 1 + Node + NodeQuad + DS + Node:DS + NodeQuad:DS + num_edges + Node:num_edges + NodeQuad:num_edges + num_nodes + Node:num_nodes + NodeQuad:num_nodes + Age, 
              data = all_data,
              random = ~ 1 + Node + NodeQuad | ID, 
              na.action = na.omit)
summary(betti_cur)
...
```{r}
betti_cur <- lme(fixed = Betti ~ 1 + Node + DS + Node:DS + SC + ST + JE num_edges + Node:num_edges + num_nodes + Node:num_nodes + Age, 
              data = all_data,
              random = ~ 1 + Node | ID, 
              na.action = na.omit)
summary(betti_cur)
