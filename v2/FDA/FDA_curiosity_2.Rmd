...
smooth.curves <- array(0, dim(betti_1))
n <- nrow(betti_1)
for(j in 1:n){
  # j = 1
  fit <- gam(betti_1[j,] ~ s(nodes, k = 10, bs = 'cr'), method = "REML")
  smooth.curves[j,] <- fit$fitted
}

matplot(nodes, t(betti_1[sel.crv,]), 
         type='l', lty=3, lwd=1, col = rainbow(3))
matlines(nodes, t(smooth.curves[sel.crv,]), 
         type='l', lty=1, lwd=1, col = rainbow(3))
```

```{r}
matplot(nodes, t(betti_1), 
        type='l', lty=1, col="light grey",
        main = "KNOT Betti (Dim 1)",
        xlab="Nodes", ylab="Betti Number")
sel.crv <- sample(1:n, size = 3, replace = FALSE)
matlines(nodes, t(betti_1[sel.crv,]), 
         type='l', lty=1, lwd=2, col = rainbow(3))
```

```{r}
mean.hat <- colMeans(betti_1)
matplot(nodes, t(betti_1), 
        type='l', lty=10, col="light grey",
        main = "KNOT Betti (Dim 1)",
        xlab="Nodes", ylab="Betti Number")
lines(nodes, mean.hat, col='blue', lwd=2)
```

```{r}
matplot(colMeans(betti_1), colMeans(compressibility), 
        type='l', lty=10, col="blue", lwd = 2,
        main = "KNOT Betti (Dim 1)",
        xlab="Betti Number", ylab="Compressibility")
```
