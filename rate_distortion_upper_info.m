...
% The variable setting determines how we choose the nodes to try combining.
% The variable num_pairs determines how many pairs of nodes to try
% combining each iteration.
%
...
%
% NOTE 1: Beginning with the full network, the algorithm works by iteratively
% clustering pairs of nodes so as to minimize the mutual information. To
% speed up the algorithm, we only consider pairs of clusters specified by
% setting.
%
% NOTE 2: The difference between this algorithm and 'rate_distortion_upper'
% is that here we return extra information about the clustering, whereas
% 'rate_distortion_upper' only returns S (to speed up computations).
